...
export default {
  ...
  methods: {
    ...
    getPortPosition(type, node) {
      const ref = this.$refs[`node${node.id}`][0];
      const x = this.scene.centerX + node.x;
      const y = this.scene.centerY + node.y;
      if (type === 'top') {
        return [x + ref.$el.clientWidth * 0.5, y];
      } else if (type === 'bottom') {
        return [x + ref.$el.clientWidth * 0.5, y + ref.$el.clientHeight];
      }
    },
    linkingStart(index) {
